...
col1.metric("Eventos qualificados", f"{len(evt_f):,}")
col2.metric("Horas paradas", f"{evt_f['periodo_h'].sum():,.2f}")
col3.metric("bbl perdidos", f"{evt_f['bbl'].sum():,.2f}")
col4.metric("Perda financeira (USD)", f"{evt_f['perda_financeira_usd'].sum():,.2f}")
...
# Rodapé
st.caption("© PRIO — Dashboard de Criticidade de Equipamentos.")
